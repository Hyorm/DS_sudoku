...
class ClauseBag {
  public:
    ...
    vector<int> star_bag[NR][NC][NR][NC];
    
    int count_clause() {
      int i, j, k, l, n, m, n_clause = 0;
      ...
      FZTN(i, NR) {
        FZTN(j, NV) {
          ...
          FZTN(k, NR) {
            FZTN(l, NV) {
              if(this->star_bag[i][j][k][l].size()) n_clause++;
            }
          }
...
void get_clause_bag(ClauseBag* cb) {
  ...
  do {
    if(map[PTOR(p)][PTOC(p)] >= '1' && map[PTOR(p)][PTOC(p)] <= '9') continue;  // pass if there is non-zero value at map(i, j)
  ...
  */
    ...
    FZTN(i, NR) {
      FZTN(j, NC) {
        FZTN(k, NR) {
          FZTN(l, NC) {
            if(cb->star_bag[i][j][k][l].size() == 0) continue;

            FZTN(n, cb->star_bag[i][j][k][l].size()) {
              fprintf(fp, "%d ", cb->star_bag[i][j][k][l][n]);
            } fprintf(fp, "0\n");
          }
        }
      }
